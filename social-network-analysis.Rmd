...
g_friendly <- set_vertex_attr(g_friendly, 
                              "gender", 
                              value = friendly$gender.from)


vertex_attr(g_friendly) 
...
friendly_edge_mat <- as.matrix(friendly_edge)

g_friendly <- graph.edgelist(friendly_edge_mat) 


# create node list

friendly_nodes <- read_csv("get.on.with.csv") 
...
V(g_friendly)$color <- ifelse(
  V(g_friendly)$gender == "male", "red", "white"
)

plot(g_friendly, vertext.label.color = "black") 
